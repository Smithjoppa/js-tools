...
(function() {
  ...
  const CLONE_SYMBOLS_FLAG = 3
	//区域
	Tools.areas = window.BEE_AREAS || {};
	//PhoneUtils命名空间
	Tools.PhoneUtils = {
		phoneRegexs: {
			//中国电信号码段
	...
	Tools.StringUtils = {
		...
		isNest: function(rule, str) {
			...
			var stack = [];
			for(var i = 0; i < len; i++) {
				if(Tools.ArrayUtils.inArray(keys, chs[i])) {
					stack.push(rule[chs[i]]);
				} else {
					if(chs[i] === stack[stack.length - 1]) {
						stack.pop();
					} else if(Tools.ArrayUtils.inArray(values, chs[i])) {
						return false;
					}
				}
			}
	...
	Tools.ObjectUtils = {
    /**
     * @param {int} bitmask 1深拷贝 2浅拷贝
     */
    baseClone:function(value, bitmask) {
      let result
      ...
      ? (isFlat ? this.getAllKeysIn : getAllKeys)
      : (isFlat ? keysIn : keys)
	...
	Tools.IdCardUtils = {
		...
		getPersonInfo18: function(idCard) {
			...
			var address = '';
			var sex = '';
			address = Tools.areas[idCard.substr(0, 2) + '0000'] + ' ' + Tools.areas[idCard.substr(0, 4) + '00'] + ' ' + Tools.areas[idCard.substr(0, 6)];
	...
	Tools.DateUtils = {
		...
		formt: function(dateTime, pattern) {
			...
			if(Tools.StringUtils.isBlank(pattern)) {
				return date.toLocaleString();
			}
			return pattern.replace(/([a-z])\1*/ig, function(matchStr, group1) {
				var replacement = "";
				switch(group1) {
					case Tools.DateUtils.patterns.PATTERN_ERA: //G
						break;
					case Tools.DateUtils.patterns.PATTERN_WEEK_YEAR: //Y
					case Tools.DateUtils.patterns.PATTERN_YEAR: //y
						replacement = date.getFullYear();
						break;
					case Tools.DateUtils.patterns.PATTERN_MONTH: //M
						var month = date.getMonth() + 1;
						replacement = (month < 10 && matchStr.length >= 2) ? "0" + month : month;
						break;
					case Tools.DateUtils.patterns.PATTERN_DAY_OF_MONTH: //d
						var days = date.getDate();
						replacement = (days < 10 && matchStr.length >= 2) ? "0" + days : days;
						break;
					case Tools.DateUtils.patterns.PATTERN_HOUR_OF_DAY1: //k(1~24)
						var hours24 = date.getHours();
						replacement = hours24;
						break;
					case Tools.DateUtils.patterns.PATTERN_HOUR_OF_DAY0: //H(0~23)
						var hours24 = date.getHours();
						replacement = (hours24 < 10 && matchStr.length >= 2) ? "0" + hours24 : hours24;
						break;
					case Tools.DateUtils.patterns.PATTERN_MINUTE: //m
						var minutes = date.getMinutes();
						replacement = (minutes < 10 && matchStr.length >= 2) ? "0" + minutes : minutes;
						break;
					case Tools.DateUtils.patterns.PATTERN_SECOND: //s
						var seconds = date.getSeconds();
						replacement = (seconds < 10 && matchStr.length >= 2) ? "0" + seconds : seconds;
						break;
					case Tools.DateUtils.patterns.PATTERN_MILLISECOND: //S
						var milliSeconds = date.getMilliseconds();
						replacement = milliSeconds;
						break;
					case Tools.DateUtils.patterns.PATTERN_DAY_OF_WEEK: //E
						var day = date.getDay();
						replacement = Tools.DateUtils.week['ch'][day];
						break;
					case Tools.DateUtils.patterns.PATTERN_DAY_OF_YEAR: //D
						replacement = Tools.DateUtils.dayOfTheYear(date);
						break;
					case Tools.DateUtils.patterns.PATTERN_DAY_OF_WEEK_IN_MONTH: //F
						var days = date.getDate();
						replacement = Math.floor(days / 7);
						break;
					case Tools.DateUtils.patterns.PATTERN_WEEK_OF_YEAR: //w
						var days = Tools.DateUtils.dayOfTheYear(date);
						replacement = Math.ceil(days / 7);
						break;
					case Tools.DateUtils.patterns.PATTERN_WEEK_OF_MONTH: //W
						var days = date.getDate();
						replacement = Math.ceil(days / 7);
						break;
					case Tools.DateUtils.patterns.PATTERN_AM_PM: //a
						var hours24 = date.getHours();
						replacement = hours24 < 12 ? "\u4e0a\u5348" : "\u4e0b\u5348";
						break;
					case Tools.DateUtils.patterns.PATTERN_HOUR1: //h(1~12)
						var hours12 = date.getHours() % 12 || 12; //0转为12
						replacement = (hours12 < 10 && matchStr.length >= 2) ? "0" + hours12 : hours12;
						break;
					case Tools.DateUtils.patterns.PATTERN_HOUR0: //K(0~11)
						var hours12 = date.getHours() % 12;
						replacement = hours12;
						break;
					case Tools.DateUtils.patterns.PATTERN_ZONE_NAME: //z
						replacement = Tools.DateUtils.getZoneNameValue(date)['name'];
						break;
					case Tools.DateUtils.patterns.PATTERN_ZONE_VALUE: //Z
						replacement = Tools.DateUtils.getZoneNameValue(date)['value'];
						break;
					case Tools.DateUtils.patterns.PATTERN_ISO_DAY_OF_WEEK: //u
						break;
					case Tools.DateUtils.patterns.PATTERN_ISO_ZONE: //X
						break;
					default:
						break;
				}
	...
	//简单四则运算
	Tools.ElementaryArithmeticUtils = {
		//运算符优先级
		operatorPrecedence: {
			'+': 0,
			'-': 0,
		...
		infixToPostfixExpression: function(expression) {
			expression = Tools.StringUtils.deleteWhitespace(expression);
			...
			for(var i = 0; i < size; i++) {
				if(Tools.StringUtils.isNumeric(elementArr[i])) {
					//如果是数值
		...
		calculateByPrefixExpression: function(expression) {
			...
			for(var i = size - 1; i >= 0; i--) {
				if(Tools.StringUtils.isNumeric(elementArr[i])) {
					...
					switch(elementArr[i]) {
						case '+':
							result = Tools.ElementaryArithmeticUtils.add(operand1, operand2);
							break;
						case '-':
							result = Tools.ElementaryArithmeticUtils.subtract(operand1, operand2);
							break;
						case '×':
						case '*':
							result = Tools.ElementaryArithmeticUtils.multiply(operand1, operand2);
							break;
						case '\/':
						case '÷':
							result = Tools.ElementaryArithmeticUtils.divide(operand1, operand2);
							break;
						default:
							result = '';
		...
		calculateByPostfixExpression: function(expression) {
			...
			for(var i = 0; i < size; i++) {
				if(Tools.StringUtils.isNumeric(elementArr[i])) {
					...
					switch(elementArr[i]) {
						case '+':
							result = Tools.ElementaryArithmeticUtils.add(operand2, operand1);
							break;
						case '-':
							result = Tools.ElementaryArithmeticUtils.subtract(operand2, operand1);
							break;
						case '×':
						case '*':
							result = Tools.ElementaryArithmeticUtils.multiply(operand2, operand1);
							break;
						case '\/':
						case '÷':
							result = Tools.ElementaryArithmeticUtils.divide(operand2, operand1);
							break;
						default:
							result = '';
							alert("The operator 【" + elementArr[i] + "】 is not legal");
	...
	Tools.RegexUtils = {
		...
		createRegexObjMustExclude: function(input, conditions) {
			...
			var subPattern = "";
			for(var i = 0; i < size; i++) {
				targetStrArr[i] = Tools.StringUtils.escapeMetacharacterOfStr(targetStrArr[i]);
				subPattern += targetStrArr[i];
				if(i != size - 1) {
					subPattern += "|";
				}
			}
			regex = Tools.StringUtils.format(regex, [subPattern]);
			switch(matcherFlag) {
				case '0':
					regex += "\\d";
					break;
			...
			regex += Tools.StringUtils.isNotBlank(length) ? "{" + length + "}" : "+";
			regex += "$";
